...
plot_struct = ABIType.from_string('(uint64,uint64,byte[10])')
...
for i in range(0, len(user_plot_box_value), plot_struct_length):
    ...
    plot_asset_id, plots_last_reward_claim_time, plot_spaces = plot_info
    print(f'User owns plot ID: {plot_asset_id}, Last Reward Claim Time is {plots_last_reward_claim_time}, Plot Spaces: {plot_spaces}')